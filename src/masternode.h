...
//
// Masternode Announce
// Used to relay new masternodes
//

class CMasterNodeAnnounce
{
    CMasterNode *pMN;

    //total number of masternodes
    int count; 
    //current masternode count
    int i; 
    ...
    (
        ...
        READWRITE(pMN.sig);
        READWRITE(pMN.now);
        READWRITE(pMN.pubkey);
        READWRITE(pMN.pubkey2);
        READWRITE(count);
        READWRITE(i);
        READWRITE(pMN.lastTimeSeen);
        READWRITE(pMN.protocolVersion);
    )

    void Relay(int countIn, int iIn){
        count = countIn;
        i = iIn;

        CInv inv(MSG_MASTERNODE_MNA, GetHash());
...
//
// Masternode Ping
// Update last seen for a specific masternode
//
...
class CMasterNodePing
{
    ...
    (
        READWRITE(pMN.vin);
        READWRITE(pMN.vchSig);
        READWRITE(pMN.pNow);
        READWRITE(stop);
    )

    void Relay(int stopIn){
        stop = stopIn;

        CInv inv(MSG_MASTERNODE_MNP, GetHash());
