...
int nAnonymizeEcashAmount = 1000;
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Ecash
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Ecash
    // Mac: ~/Library/Application Support/Ecash
    // Unix: ~/.ecash
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Ecash";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "ecash.conf"));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good())
        return; // No ecash.conf file is OK
    ...
    {
        // Don't overwrite existing settings so command line settings override ecash.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "ecashd.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong Ecash will not work properly.");
